...
pub(crate) const INSPECTOR_SAMPLE_HOLD: Duration = Duration::from_millis(150);
...
fn inspector_display_state(
    ...
    } else if has_current_sample {
        InspectorDisplayState::Ready
    } else if has_held_sample {
...
pub(crate) struct CellInspectorCollector {
    ...
    hovered_cell: Option<CellCoordinate>,
    presented_sample: Option<CellInspectorSample>,
    hold_expires_at: Option<Instant>,
...
impl CellInspectorCollector {
    ...
    pub(crate) fn set_hover_at(&mut self, hovered_cell: Option<CellCoordinate>, now: Instant) {
        ...
        let previous_hover = self.hovered_cell;
        self.hovered_cell = hovered_cell;
        self.selection_generation = self.selection_generation.wrapping_add(1);

        if hovered_cell.is_none() || !self.world_ready {
            self.presented_sample = None;
            self.hold_expires_at = None;
            return;
        }

        let Some(sample) = self.presented_sample.as_ref() else {
            self.hold_expires_at = None;
            return;
        };
        if sample.world_epoch != self.world_epoch {
            self.presented_sample = None;
            self.hold_expires_at = None;
        } else if Some(sample.cell) == hovered_cell
            && sample.selection_generation == self.selection_generation
        {
            self.hold_expires_at = None;
        } else if Some(sample.cell) == previous_hover {
            // Start one bounded hold when leaving the Cell that produced the
            // presented sample. Further rapid hover changes preserve this
            // sample and deadline rather than alternating through Sampling.
            self.hold_expires_at = Some(now + INSPECTOR_SAMPLE_HOLD);
        }
    }
    ...
    pub(crate) fn hud_data(&self, current_simulation_tick: u64, now: Instant) -> InspectorHudData {
        let current_sample = self.presented_sample.as_ref().filter(|sample| {
            Some(sample.cell) == self.hovered_cell
                && sample.world_epoch == self.world_epoch
                && sample.selection_generation == self.selection_generation
        });
        let held_sample = self.presented_sample.as_ref().filter(|sample| {
            self.hovered_cell.is_some()
                && current_sample.is_none()
                && sample.world_epoch == self.world_epoch
                && self
                    .hold_expires_at
                    .is_some_and(|expires_at| now < expires_at)
        });
        let displayed_sample = current_sample.or(held_sample);
        let sample_tick_is_future =
            displayed_sample.is_some_and(|sample| sample.simulation_tick > current_simulation_tick);
        let sample_age_ticks = displayed_sample
            .and_then(|sample| current_simulation_tick.checked_sub(sample.simulation_tick));
        let sample_age_millis = displayed_sample.map(|sample| {
            u64::try_from(
        ...
        InspectorHudData {
            ...
            sample: displayed_sample.cloned(),
    ...
    pub(crate) fn update(
        ...
        let needs_sample = self.presented_sample.as_ref().is_none_or(|sample| {
            sample.cell != cell
                || sample.world_epoch != self.world_epoch
                || sample.selection_generation != self.selection_generation
    ...
    fn consume_mapped(&mut self, now: Instant) -> Result<(), CellInspectorReadbackError> {
        ...
        self.hold_expires_at = None;
        self.presented_sample = Some(CellInspectorSample {
            cell: identity.cell,
...
mod tests {
    ...
    #[test]
    fn presentation_state_separates_sampling_held_ready_and_failure() {
    ...
    #[test]
    fn gpu_collector_holds_one_presented_sample_across_rapid_hovers_and_reuses_after_reset() {
        #[derive(Clone, Copy)]
        struct ProbeValues {
            material: u32,
            temperature: f32,
        ...
        let fourth = CellCoordinate { x: 31, y: 77 };
        let wood_flags = with_fuel_progress(FLAG_COMBUSTING | FLAG_FLAME_EVENT, 438);
        write_probe(
            &simulation,
            first,
        ...
        let held = collector.hud_data(8, started + Duration::from_millis(103));
        assert_eq!(held.display_state, InspectorDisplayState::Held);
        let held_sample = held.sample.as_ref().expect("held presented sample");
        assert_eq!(held.hovered_cell, Some(second));
        assert_eq!(held_sample.cell, first);
        assert_eq!(held_sample.material_id, MATERIAL_WOOD);
        assert_eq!(held_sample.simulation_tick, 0);
        assert_eq!(held_sample.diagnostic_sequence, 1);
        assert_eq!(held.sample_age_ticks, Some(8));
        assert_eq!(
            collector
                .hud_data(8, started + Duration::from_millis(252))
                .display_state,
            InspectorDisplayState::Sampling,
            "the presented-sample hold expires at 150 ms"
        );
        assert!(collector.pending.is_some());
        collector.set_hover_at(Some(third), started + Duration::from_millis(104));
        let third_held = collector.hud_data(8, started + Duration::from_millis(105));
        assert_eq!(third_held.display_state, InspectorDisplayState::Held);
        assert_eq!(third_held.hovered_cell, Some(third));
        assert_eq!(
            third_held.sample.as_ref().map(|sample| sample.cell),
            Some(first)
        );
        collector.set_hover_at(Some(fourth), started + Duration::from_millis(106));
        let fourth_held = collector.hud_data(8, started + Duration::from_millis(107));
        assert_eq!(fourth_held.display_state, InspectorDisplayState::Held);
        assert_eq!(fourth_held.hovered_cell, Some(fourth));
        assert_eq!(
            fourth_held.sample.as_ref().map(|sample| sample.cell),
            Some(first)
        );
        simulation
            .context
        ...
        assert!(collector.presented_sample.is_some());
        assert!(collector.hold_expires_at.is_some());
        assert_eq!(
            collector
                .hud_data(8, started + Duration::from_millis(108))
                .display_state,
            InspectorDisplayState::Held
        );
        collector
        ...
        let sample = collector
            .presented_sample
            .as_ref()
            .expect("new Cell sample");
        assert_eq!(sample.cell, fourth);
        assert_eq!(sample.material_id, MATERIAL_STONE);
        ...
        assert!(collector.hold_expires_at.is_none());
        assert_eq!(
            collector.hud_data(0, started).display_state,
            InspectorDisplayState::Sampling
        );
