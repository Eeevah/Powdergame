...
- G8: Performance Evidence — IN_PROGRESS
  ...
  - G8-B Benchmark Scenario Suite: IMPLEMENTATION CANDIDATE; Scenarios 1 Sand Fall, 2 Water Flow, 3 Fire / Heat, and 4 Pressure Burst USER ACCEPTED; Water and Pressure automatic NEEDS_HUMAN_REVIEW unchanged / known follow-up; Fire automatic PASS unchanged; Cell Inspector v0 source `a2aac71` IMPLEMENTATION CANDIDATE / USER ACCEPTANCE PENDING; Scenario 5 Heavy Mixed World PENDING — DO NOT START; overall NOT CLOSED
...
25. Cell Inspector v0 — source `a2aac71219e145b84d6dd050865a81631da490bd` **IMPLEMENTATION CANDIDATE / USER ACCEPTANCE PENDING**
26. 같은 source SHA의 G8-A user visual validation
27. Heavy Mixed World는 **PENDING — do not start**; G8-C는 이후 별도 사용자 지시 전 시작 금지
28. 사용자 결정 B: G9 Playable First World 진행
29. 사용자 결정 C: M0 승인 이후에만 P1 identity/descriptor 등록 검토
30. M0 승인 후 M1 Interaction Grammar Alpha 설계 확정
...
- Heavy Mixed World — **PENDING — DO NOT START / NOT YET USER ACCEPTED**
...
Workspace fmt and affected Windows check passed; Windows binary tests passed `104` with `1` intentional ignore; Inspector targeted tests passed `15/15`; affected-package all-target clippy and strict policy audit passed. Exactly one 3-frame Gallery smoke passed; workspace FULL count is `0` and scenario candidate count is `0`. Screenshot-backed manual UX checked Sand, Water, Pressure, reset, and scenario switching. This evidence does not replace user acceptance. Heavy Mixed World is **PENDING — do not start**, and G8-B remains **NOT CLOSED**. The source of truth for the UI contract and acceptance checklist is `docs/vision/UI_DIRECTION.md`.
