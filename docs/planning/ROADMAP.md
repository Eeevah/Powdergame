...
G8-A는 2048×2048 reference world와 실제 production pass를 측정할 수 있는 기반을 만들었다.
...
## 4. Immediate Prerequisite — Canonical Recovery

현재 `main`의 연구/문서 진행선과 `feature/m0-g8-performance-evidence`의 구현/증거 진행선이 분기되어 있다.

다음 제품 작업을 시작하기 전에 하나의 buildable canonical line으로 통합한다.
...
1. **Canonical Recovery** — 구현선과 연구/문서선을 하나의 buildable `main`으로 통합한다.
2. **G8-B** — 다섯 benchmark fixture를 구현한다.
3. **G8-C** — 공식 multi-trial matrix와 병목 결론을 기록한다.
4. **G9-A** — 실제 sandbox input/edit loop를 만든다.
5. **G9-C / G9-D** — 최소 Discovery와 정직한 Presentation feedback을 연결한다.
6. **G9-E** — 사용자가 자유롭게 플레이하고 M0 승인 여부를 결정한다.
7. M0 승인 후에만 **M1 Interaction Grammar Alpha**를 정식 Evidence Gate로 확정한다.
