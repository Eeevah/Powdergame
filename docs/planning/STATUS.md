...
- **G8-B (Benchmark Scenario Suite)**: **SCENARIOS 1–4 USER ACCEPTED; SCENARIO 5 HEAVY MIXED WORLD NEXT / NOT CLOSED**
...
- **G8-B Pressure Burst causal remediation**: **CLEAN-SOURCE CANDIDATE AUTOMATIC NEEDS_HUMAN_REVIEW UNCHANGED / USER ACCEPTED WITH KNOWN FOLLOW-UP / HEAVY MIXED NEXT**
...
1. **Heavy Mixed World**: Scenarios 1–4 acceptance 뒤 남은 Scenario 5의 fixture/Harness/user review가 G8-B의 다음 작업이다.
2. **Scope boundary**: G8-C, G9, optimization, `main`/PR은 별도 사용자 지시 전 시작하지 않는다. Accepted Pressure candidate/artifacts와 automatic verdict는 변경하거나 재실행하지 않는다.

---

### Blockers

Canonical Recovery, v5 technical evidence, Sand Fall behavior, Sand Fall Harness pilot, Water Flow user acceptance, Fire / Heat automatic candidate와 user acceptance, Pressure Burst causal candidate와 user acceptance에는 확인된 기술 blocker가 없다. Water의 local-liquid 재배열과 Pressure의 top-seam-only opening/small plume/broad terminal activity는 알려진 후속 과제이며 production-physics defect 증거는 없다. Pressure tail/activity 비용은 G8-C에서 측정한다. G8-B closure에 남은 acceptance는 Scenario 5 Heavy Mixed World다. G8-A의 same-SHA visual validation, `main` 승격과 G8-C/G9 진입은 별도 사용자 결정이다.
...
M0 implementation: **IN_PROGRESS** — G0/G1/G2/G3/G4/G5/G6/G7 PASS / CLOSED (G2/G3/G4/G5/G6/G7 User Validation APPROVED); G8 Performance Evidence: IN_PROGRESS (G8-A verified v5 evidence candidate; official capture and independent verification complete; same-SHA User Visual Validation PENDING; G8-B Scenarios 1–4 user accepted, Water and Pressure automatic `NEEDS_HUMAN_REVIEW` unchanged with known follow-ups, Scenario 5 Heavy Mixed World next, overall NOT CLOSED; G8-C PENDING).
...
m0_status: IN_PROGRESS (G0-G7 PASS/CLOSED User Validation APPROVED; G8 IN_PROGRESS — G8-A verified v5 evidence candidate, same-SHA user visual validation pending; G8-B scenarios 1-4 accepted, Water and Pressure automatic NEEDS_HUMAN_REVIEW unchanged with known follow-ups, Scenario 5 Heavy Mixed World next, overall not closed; G8-C pending)
```
