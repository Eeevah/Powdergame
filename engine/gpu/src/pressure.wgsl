// TE-5R1 post-phase Steam-load relaxing dynamic pressure.
...
@group(0) @binding(4) var<storage,read_write> pressure_next:array<f32>;
@group(0) @binding(5) var<storage,read> movement_class_table:array<u32>;
@group(0) @binding(6) var<storage,read> chunk_state:array<u32>;
const EMPTY:u32=0u;const STEAM:u32=6u;const TABLE_LEN:u32=16u;const LIQUID:u32=2u;const GAS:u32=3u;
const D:f32=0.20;const R:f32=0.02;const LV:f32=480.0;const FULL:f32=100.0;const PMAX:f32=1.0e6;
fn finite(v:f32)->bool{return v==v&&abs(v)<=1.0e20;}
fn clean(v:f32)->f32{return clamp(select(0.0,v,finite(v)),0.0,PMAX);}
...
@compute @workgroup_size(64,1,1) fn pressure_main(@builtin(global_invocation_id) gid:vec3<u32>){let i=gid.y*params.threads_x+gid.x;if(i>=params.cell_count){return;}let m=material_current[i];if(params.sleep_enabled!=0u){let cx=(i%params.width)/params.chunk_size;let cy=(i/params.width)/params.chunk_size;if(chunk_state[cy*params.chunks_x+cx]!=0u){pressure_next[i]=select(0.0,clean(pressure_current[i]),node(m));return;}}pressure_next[i]=predicted(i);}
