...
function Get-ChangedPaths {
    ...
    foreach ($range in @("$FromRef...HEAD", "$FromRef..HEAD")) {
        $result = Invoke-RepoGit -GitArgs @("diff", "--name-only", "--diff-filter=ACMR", $range) -AllowFailure
    ...
    if (-not $rangeFound) {
        $result = Invoke-RepoGit -GitArgs @("diff", "--name-only", "--diff-filter=ACMR", "HEAD^", "HEAD") -AllowFailure
        if ($result.ExitCode -eq 0) {
            foreach ($line in $result.Text -split "`r?`n") {
                if ($line) { [void]$paths.Add($line.Replace("\", "/")) }
            }
